...
import { ShowCertificate } from '../components'
  ...
  return (
    <div className='h-[90%] w-full flex justify-start items-center bg-bg-primary gap-4 flex-col py-5 px-5'>
        <div className='w-full flex justify-between items-center px-5'>
          ...
          {certificateData.data.map((item,index) => (
              <UserCertificate toggle = {toggle} setToggle = {setToggle} data = {item}/>
          ))}
          </div>
        
        {
          toggle && <ShowCertificate setToggle = {setToggle}/>
        }
    </div>
  )
}
